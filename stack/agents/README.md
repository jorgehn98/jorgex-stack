...
- El flujo de trabajo lo define la skill `work-lifecycle`: `work/{nombre}/plan.md` es el tablero de estado y se mantiene entre merges intermedios; cada tarea formal declara en su columna `Spec` una única fuente recuperable: una observación Engram (`work/{nombre}/task/{NN}`) o Markdown canónico (`work/{nombre}/tasks/{NN}.md`). Los resultados de fase (`work/{nombre}/{fase}`), los checkpoints de PR (`work/{nombre}/pr/{NN}`) y el cierre final (`work/{nombre}/done`) viven en Engram. Los subagentes reciben el título y la referencia `Spec` exacta; un mensaje directo o inline solo es un microencargo auxiliar de su tarea padre y, si crece o se independiza, debe persistirse antes de continuar.

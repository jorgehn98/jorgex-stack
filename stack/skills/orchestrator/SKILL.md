...
- The full spec of each formal task has one declared recoverable source: an Engram observation under `work/{name}/task/{NN}` with verified identity/access, or canonical Markdown at `work/{name}/tasks/{NN}.md`. Record it in the plan's `Spec` column; existing Engram specs remain valid and need no migration. When you delegate, pass the title and exact `Spec` reference as read-only for the worker so the subagent resolves it directly.

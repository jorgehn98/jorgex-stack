...
Conflict handling after \`mem_save\`:
- If \`mem_save\` returns \`judgment_required: true\` or \`candidates[]\`, review each candidate.
- Use \`mem_judge\` with \`candidates[].judgment_id\` for clear \`related\`, \`compatible\`, \`scoped\`, or \`not_conflict\` cases.
- Ask the user before marking \`supersedes\` or \`conflicts_with\` for architecture, policy, or decision memories, or whenever confidence is low.

### WHEN TO SEARCH MEMORY

Use the global \`engram\` subagent whenever memory needs to be read, searched, loaded, recalled, or synthesized. The main agent should not pull broad memory directly unless it is a tiny one-off lookup; delegate to \`engram\` so it can gather all needed memories, curate/process them, and return only the minimal relevant output without polluting the main agent context. Saving memory is still done by the main agent with \`mem_save\` / \`mem_session_summary\`.

When the user asks to recall something — any variation of "remember", "recall", "what did we do",
"how did we solve", "recordar", "acordate", "qué hicimos", or references to past work:
1. Delegate to \`engram\` with a concise request, e.g. \`carga contexto [task]\` or \`busca: [topic]\`.
2. Let \`engram\` decide whether to use \`mem_context\`, \`mem_search\`, and \`mem_get_observation\`.
3. Use only the curated output returned by \`engram\` unless a tiny direct lookup is clearly enough.

Also search memory PROACTIVELY when:
- Starting work on something that might have been done before
- The user mentions a topic you have no context on — check if past sessions covered it
- The user's FIRST message references the project, a feature, or a problem — ask \`engram\` to check prior work before responding
...
2. Then delegate to the global \`engram\` subagent to recover additional context from previous sessions

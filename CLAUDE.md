...
- ✅ F5.5 (v0.6.0): hardening tras revisión de gentle-ai + caza de bugs — escrituras atómicas (temp+rename), manifest de instalación en ~/.jorgex-stack/manifest.json (sync limpia huérfanos de versiones previas, uninstall borra exacto; limpieza deshabilitada si algún plan falla), reparación de marcadores markdown rotos (incl. inline/duplicados, sin pérdida de contenido), dedup de backups por checksum compuesto, TOML robusto (headers con comentario/variantes, strings multilínea), delegation triggers en orchestrator, plugin engram.ts parametrizado vía "{{ENGRAM_BIN}}" (cero rutas personales en el repo), commands x-machine-specific filtrados (video.md no se instala), hooks sin {{SCRIPTS_DIR}} identificados por comando literal, retirada del MCP engram redundante cuando aparece el plugin oficial (Claude/Codex), allowlists de memoria con doble namespace (mcp__engram__* y mcp__plugin_engram_engram__*), keys: valor del usuario > env var, fixes CLI (comando default roto, update propaga --target-dir/--dry-run/exit code, main catch, uninstall sin runtimes avisa), post-pr-review canal único por runtime (sin duplicar en OpenCode), ~/.claude.json nunca se borra en uninstall, lookPath Windows (.exe antes que shim sh), samePath case-insensitive + env vars (OPENCODE_CONFIG_DIR/CODEX_HOME) en derivación de ~/.agents. 50 tests. Limitación documentada: renombrar un script de hooks canónico deja la entrada vieja en settings.json/hooks.json del usuario (migrar a mano cuando ocurra).

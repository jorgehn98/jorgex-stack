...
> **TL;DR**
>
> - **Recomendado**: Playwright CLI global (`@playwright/cli`), con release estable resuelto desde npm y verificado antes de instalar. No añade schemas MCP permanentes. La guía browser se proyecta condicionalmente tras la activación explícita; indica abrir Chromium con `--browser=chromium`, consultar `playwright-cli --help`, usar una sesión con nombre, obtener refs con `snapshot`, verificar resultados y cerrar solo la sesión creada.
> - **Avanzado opt-in**: Chrome DevTools MCP en modo **full** (~29 tools, ~5,8–7,7k tokens de schemas; cifra dependiente del release). Default-off, selección por runtime, paquete resuelto y verificado, argv `--isolated --redact-network-headers --no-performance-crux --no-usage-statistics`: Chrome se levanta con un **perfil temporal aislado, eliminado al cerrar** (no hay perfil persistente dedicado), las cabeceras sensibles se redactan, pero los cuerpos de request/response pueden contener tokens o PII; evita sesiones autenticadas o datos sensibles. CrUX + telemetría están deshabilitados.
> - **Pi**: el selector activa las integraciones solo cuando el candidato declara la capability correspondiente. El paquete Pi compatible publicado admite versiones observadas en los handoffs `PI_CODING_AGENT_DIR/jorgex-pi/playwright.v1.json` y `devtools.v1.json`. Context7 usa `context7-http-v1`: Pi registra un bridge HTTP aislado en memoria durante el bootstrap y no escribe configuración MCP ni credenciales. `available` permite el registro, pero no implica un handshake HTTP; una colisión conserva el archivo MCP y bloquea la activación gestionada. Los handoffs quedan registrados con SHA-256 en el receipt de proyección y se eliminan solo tras volver a comprobar su integridad.
...
| Pieza | Origen | Versión |
|---|---|---|
| Paquete `@playwright/cli` | npm (gestión global con pnpm) | release estable `dist-tags.latest` verificado; versión e integridad observadas en `~/.jorgex-stack/playwright-cli.json` |
...
Stack valida primero la metadata oficial y el SRI del tarball correspondiente. Después instala la versión exacta con `pnpm add --global @playwright/cli@<versión-observada>` y Chromium con `pnpm dlx @playwright/cli@<versión-observada> install-browser chromium` (argv directo, sin alias flotante en la ejecución). Comprueba la versión instalada y el arranque del navegador antes de activar la guía.
...
El paquete global se instala por su **versión exacta** tras la verificación del tarball; la descarga de Chromium usa esa misma versión con `pnpm dlx @playwright/cli@<versión-observada> install-browser chromium`. `pnpm add --global` registra el binario en el `PATH` global del usuario. Stack comprueba el manifest instalado y el arranque de Chromium antes de guardar la observación; la verificación previa del SRI no equivale a comparar los bytes de la instalación global con los del stage.
...
Ambos respetan la regla #1 del repo (**pnpm siempre, nunca npm**). Si `pnpm` no se resuelve (`lookPath("pnpm")` falla), el plan entero aborta con código no-cero y la preferencia no se persiste. Si falla una instalación autorizada, el error identifica la fase — preflight de pnpm, paquete global, descarga del navegador, verificación del ejecutable en `PATH` o persistencia de la preferencia — y recomienda reintentar con `jorgex-stack install --playwright`; la preferencia solo se marca como habilitada tras completar todas las fases.
...
| `doctor` | Reporta el estado como `disabled` (sin preferencia), `healthy` (CLI + caché de Chromium listos y arranque headless de `about:blank` verificado), `missing:package` / `missing:browser` (preferencia activa pero algo falta), `unreadable` (la ruta de caché no se puede leer), `not-in-path` (el paquete está en el directorio global de pnpm pero esta terminal aún no heredó el `PATH`), `broken` (Chromium no arranca o el binario no responde) u `outdated` (versión distinta de la observación guardada). Cuando la preferencia está activa y el paquete y la caché están presentes, `doctor` ejecuta una única comprobación local de arranque headless contra `about:blank`; no abre sitios externos ni repara estado. La comprobación de caché conserva la ruta resuelta para `missing` y `unreadable`, además del código de filesystem cuando existe (`ENOENT`, `DISABLED`, `EACCES`, etc.); el estado `unreadable` de `doctor` expone ruta y código. Si informa `not-in-path`, abre una terminal nueva después de `pnpm setup`; no hace falta reinstalar Playwright. Si `~/.jorgex-stack/playwright-cli.json` o `~/.jorgex-stack/devtools-mcp.json` están ilegibles, imprime la ruta exacta y `Corrige o borra ese archivo antes de reintentar` antes de evaluar el resto del estado browser; los counts de "problemas" suben por cada archivo corrupto. |
| `update --check` | Solo inspecciona Playwright CLI si la preferencia está `enabled` (un binario casual en `PATH` no es consentimiento). Compara la versión detectada con la **observación local**, sin consultar un release nuevo ni descargar; **no** requiere `sync` después. Si alguna preferencia de navegador está corrupta, aborta con exit 1 antes de imprimir nada. |
| `update` interactivo | Solo ofrece Playwright tras consentimiento previo: consulta la versión pública para informar drift; si falla la consulta, avisa que no pudo comprobar si existe una versión nueva. Si se selecciona con una segunda confirmación explícita, resuelve y verifica metadata/SRI del candidato exacto. Aplica tanto el paquete global como la descarga de Chromium con ese mismo candidato, comprueba el arranque y persiste su versión observada. Antes del plan global corre `pnpm bin --global`; si falla, no reescribe el paquete y muestra el remedio `pnpm setup` + terminal nueva + reintento con `jorgex-stack update`. No exige `sync` posterior (`resolveUpdateSyncRequired` solo dispara para cambios en `stack` o `skill:*`). |
...
- Los planes pnpm globales (`pnpm add --global`, `pnpm dlx`, `pnpm remove --global`) nunca se ejecutan aunque el flag `--playwright` o `--remove-playwright` esté presente; la selección de un candidato publicado tampoco se ejecuta bajo `--target-dir`.
...
- **`--target-dir --devtools`** (simulación explícita): solo materializa el servidor si se inyecta una observación validada en el harness; sin ella falla cerrado. No consulta npm ni preferencias reales, no instala Chrome y no escribe fuera del target temporal.
...
El preparador histórico de adopción no selecciona versiones para instalaciones nuevas de Pi. El runtime gestionado observa el candidato publicado y valida el handoff con el paquete/receipt verificados; una capability nueva exige publicar primero un paquete Pi compatible.
...
pnpm dlx jorgex-stack sync --devtools          # reutiliza la observación ya verificada
...
`--devtools` + `--no-devtools` juntos falla con mensaje claro. En un `install` real con opt-in, Stack resuelve el release estable `dist-tags.latest` de npm, verifica identidad y SRI del tarball y lo instala en un stage privado con `pnpm add --ignore-scripts`. Antes de escribir configuración, handoff o preferencia, ejecuta `--help` con los cuatro flags y comprueba sus **efectos reales** en el parser del paquete (`isolated=true`, `redactNetworkHeaders=true`, `performanceCrux=false`, `usageStatistics=false`), sin abrir Chrome; `--help` solo no basta, porque incluso un flag desconocido devuelve éxito. Si el paquete cambia su parser o sus flags, falla cerrado. Guarda versión e integridad observadas por máquina; `sync`, dry-run y `--target-dir` no descargan ni ejecutan el candidato, y `sync --devtools` necesita una observación previa. En Pi la selección se guarda solo después de una proyección correcta; un handoff ajeno o cuyo contenido no coincide con el SHA registrado bloquea la reconciliación y conserva el archivo para revisión. Al desactivar, Stack vuelve a comprobar el SHA antes de limpiar el handoff. Pi debe recargarse para que el bootstrap recoja el cambio.

### 3.3 Comando y argumentos

El comando proyectado usa la **versión observada** del release verificado (no el alias `latest` en runtime) y conserva los flags fijos de privacidad:

```powershell
pnpm dlx chrome-devtools-mcp@<versión-observada> \
  --isolated \
...
Sin keys ni capabilities experimentales habilitadas por defecto. El manifiesto `stack/mcp/servers.json` contiene un marcador de versión que se sustituye solo tras la verificación; los cuatro flags y su orden permanecen fijos, sin overrides desde el CLI de Stack. La prueba aislada valida los valores parseados sin abrir Chrome; no acredita compatibilidad de todas las tools ni autoriza inspeccionar sesiones autenticadas.
...
DevTools MCP se reconcilia como cualquier servidor opcional del manifiesto `stack/mcp/servers.json`. Una instalación antigua gestionada con `chrome-devtools-mcp@1.6.0` y sin observación puede desactivarse o desinstalarse sin descargar nada: solo se retira si la entrada coincide exactamente con la forma histórica owned; si fue editada, se conserva.
...
| `doctor` dice `outdated` | `@playwright/cli` no coincide con la versión observada o falta una observación válida | Ejecuta `update` interactivo (TTY) o `install --playwright` para resolver y comprobar el release publicado. |
...
| `playwright-cli: command not found` desde un subagente | El binario no está en `PATH`. Antes de reinstalar, valida que pnpm pueda resolver su bin global | Ejecuta `pnpm bin --global`; si falla, acepta `pnpm setup` en una instalación interactiva o ejecútalo manualmente y abre una terminal nueva. Luego reintenta `jorgex-stack install --playwright`, que resolverá y verificará el release publicado antes de instalarlo. La guía condicional no limita los permisos efectivos del runtime. |
| `install --devtools` falla durante la prueba de flags | El candidato publicado no expone los cuatro flags o no permite instalar/probar el CLI aislado | No se habilita ninguna preferencia o config por ese intento; revisa el error y vuelve a intentar cuando el proveedor publique un candidato compatible. No fijes manualmente un alias flotante en el manifiesto. |
...
- Herramienta global: `src/lib/external-tools.ts` → `PLAYWRIGHT_CLI` (identidad `@playwright/cli`, binario `playwright-cli`); `src/lib/browser-provider.ts` verifica releases y flags DevTools.
- Manifiesto MCP: `stack/mcp/servers.json` → `servers.chrome-devtools` (marcador `chrome-devtools-mcp@{{VERSION}}`, materializado con la observación verificada; flags de privacidad fijos).
- Secciones gestionadas del system prompt: fuentes `stack/system-prompt/context7.md`, `browser-playwright.md` y `browser-chrome-devtools.md`; montaje en `src/components/system-prompt.ts` vía `upsertMarkdownSection`/`removeMarkdownSection`; Pi compatible consume el formato modular y añade Context7 desde su bootstrap nativo mediante un bridge HTTP aislado en memoria. Ver §2.7 para el ciclo de vida.
...
- Contratos RED/GREEN cubiertos en `tests/browser-provider-resolution.test.ts` (metadata, SRI, stage y smoke de flags), `tests/external-tools.test.ts`, `tests/playwright-lifecycle.test.ts`, `tests/devtools-mcp.test.ts`, `tests/pi-managed-runtime.test.ts` (opt-in/handoff Pi), `tests/cli-mode-resolution.test.ts`, `tests/playwright-update.test.ts` (realinea `update` + `install-browser`), `tests/playwright-uninstall.test.ts` (`outro` correcto en errores de `remove`), `tests/playwright-windows-execution.test.ts` (shims `.cmd` por `cmd.exe` sin shell) y `tests/browser-preferences-safety.test.ts` (estado real aislado bajo `--target-dir` y preferencias corruptas bloquean mutaciones).

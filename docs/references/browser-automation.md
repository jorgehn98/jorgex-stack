...
Pi 0.8.19 consume `modular-system-prompts-v1`: su adapter conserva los módulos canónicos separados y, mientras el registro HTTP no se adopte, filtra `context7` de la proyección de Pi. El asset Context7 está validado, pero todavía no se registra ni se proyecta el servidor MCP en Pi; esta disponibilidad queda pendiente del checkpoint HTTP correspondiente. Playwright y DevTools mantienen sus bloques propios. El bloque legacy `jorgex:browser` se conserva como formato de entrada migrable y para cleanup/rollback compatibles. El contenido del usuario fuera de los marcadores se conserva.
...
Las fuentes canónicas viven en `stack/system-prompt/context7.md`, `browser-playwright.md` y `browser-chrome-devtools.md`; los adapters generan sus bloques mediante `upsertMarkdownSection` y aplican la inversa en `planUnmerge`. Pi 0.8.19 requiere el contrato modular, proyecta Playwright y DevTools en sus módulos propios y deja Context7 sin proyectar hasta su registro HTTP. El bloque legacy `jorgex:browser` se conserva como formato de entrada migrable y para cleanup/rollback.
...
- Secciones gestionadas del system prompt: fuentes `stack/system-prompt/context7.md`, `browser-playwright.md` y `browser-chrome-devtools.md`; montaje en `src/components/system-prompt.ts` vía `upsertMarkdownSection`/`removeMarkdownSection`; Pi 0.8.19 conserva el formato modular y filtra Context7 hasta su registro HTTP. Ver §2.7 para el ciclo de vida.

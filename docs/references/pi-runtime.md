...
| --- | --- | --- |
| System prompt | `~/.pi/agent/AGENTS.md` | Stack, en secciones marcadas `jorgex:system-prompt`, `jorgex:engram-protocol`, `jorgex:playwright` y `jorgex:chrome-devtools`; el `AGENTS.md` estático no incluye Context7 y Pi 0.8.22 lo añade desde el bootstrap, conservando el bloque legacy como entrada migrable |
...
El `AGENTS.md` estático proyectado por Stack no contiene Context7. Pi 0.8.22 añade esa sección desde su bootstrap nativo después de registrar el bridge HTTP aislado; una entrada `available` permite el registro, pero no acredita un handshake HTTP.

...
| --- | --- | --- |
| System prompt | `~/.pi/agent/AGENTS.md` | Stack, en secciones marcadas `jorgex:system-prompt`, `jorgex:engram-protocol`, `jorgex:playwright` y `jorgex:chrome-devtools`; el `AGENTS.md` estático no incluye Context7 y Pi 0.8.20 lo añade desde el bootstrap, conservando el bloque legacy como entrada migrable |
...
```text
node .github/scripts/prepare-pi-adoption.mjs --pi-dir ABS --version EXACT [--accept-pi-version 0.85.1] [--accept-playwright-skill-removal] [--accept-modular-system-prompts] [--accept-context7-http] [--apply]
```

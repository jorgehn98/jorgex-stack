...
The install journal is written before Pi runs and promoted to `installed` only after the runner reports healthy. The schema v1 receipt consumed by `jorgex-pi@0.2.2` records the complete candidate evidence, its scope, and the verified Engram executable in `engram.binary`:

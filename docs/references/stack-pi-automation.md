...
La transición Engram child-only requiere además `--accept-engram-child-only`; sin esa opción se rechaza por defecto. Solo acepta el paso exacto en `contract/runtime-agents.v1.json` desde un `engram` sin el campo a `engram.subagentOnlyExtensions=[../extensions/engram-child.ts]`. No acepta cambios en otros agentes ni en otras claves del contrato. El preparador exige que el inventario del tarball anterior se conserve y que el nuevo añada únicamente `package/extensions/engram-child.ts`; además compara los bytes del módulo empaquetado con los bytes del productor y mantiene las comprobaciones de tamaño, SRI, SHA e integridad del tarball. El flag no es un bypass, no instala el Engram oficial y no cierra por sí solo Stack #155.
...
Para la adopción Engram child-only, el run `35536946939` falló cerrado antes de crear la PR porque requería la revisión contractual. Pi `0.8.27` ya estaba publicado y verificado; por tanto, la recuperación usa el preparador local desde un checkout limpio de Stack y el checkout productor de Pi, con la versión exacta publicada y los flags que describen las transiciones presentes, incluido `--accept-engram-child-only`. Después se abre una PR manual con la base actual y se ejecutan review y gates. No se vuelve a publicar Pi, no se editan pines ni hashes manualmente y no se convierte esta recuperación en una adopción oficial completa o una instalación personal.

La automatización pasa permanentemente `acceptEngramChildOnly` al preparador de adopción para poder reconciliar esa transición cuando el diff real coincide; esa configuración permanente no fuerza la aceptación: el preparador solo activa la transición cuando el estado anterior carece del campo y el estado nuevo contiene exactamente la lista indicada. Una vez adoptado el campo, un cambio posterior o una transición inversa vuelve a requerir revisión manual.

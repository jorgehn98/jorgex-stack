...
La retirada de la skill Playwright del paquete Pi requiere además `--accept-playwright-skill-removal`; sin esa opción el preparador la rechaza por defecto. La opción solo acepta una entrada `playwright-cli` en la paridad con `sourcePath` y `targetPath` canónicos, ausente tanto de la fuente Stack fusionada como del checkout productor de Pi. Para proteger la topología del cambio, descarga el tarball anterior y valida sus hashes fijados, y exige que el inventario del nuevo tarball sea idéntico salvo por `package/skills/playwright-cli/**`; el handoff Playwright, el SDK, las dependencias y el resto de contratos se conservan. Esta aceptación es manual porque la comprobación necesita coordinar ambos checkouts y el artefacto publicado, no solo leer el diff de una rama.

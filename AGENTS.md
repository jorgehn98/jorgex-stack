...
El pin de este checkout es el publicado y verificado `npm:jorgex-pi@0.8.6`; `package.json` identifica la versión de Stack. La publicación de Stack usa el auto-bump existente al mergear y selecciona el patch disponible; no se debe afirmar ni preparar manualmente un número de release futuro. `Stack 1.9.17` reconoce el receipt de Pi `0.8.5`; la nueva versión publicada de Stack debe comprobarse en el registro y en su pin antes de consumirla, y es la que debe reconocer el receipt de `0.8.6`. La barrera gestionada de 24 h en npm afecta al consumo/instalación real del paquete Pi nuevo, no a validación, merges ni releases. Tamaño y hashes se verifican según `src/lib/pi-runtime.ts` y `tests/fixtures/pi-runtime.ts`; la attestation externa de npm queda fuera del runtime y `provenance.commit` es informativo salvo verificación explícita.

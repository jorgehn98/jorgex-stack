...
Stack gestiona la instalación de JorgeX Pi; Pi posee su proyección Pi-native, bootstrap, contratos, dependencias y lifecycle. `src/lib/pi-runtime-pin.json` es la autoridad de la identidad del paquete, procedencia y digests del tarball; `src/lib/pi-runtime.ts` conserva el lifecycle y sus contratos. Para el paquete `jorgex-pi`, nunca usar `latest`: solo se consumen versiones exactas que reconozcan cada receipt. El binario Engram es la excepción documentada: una instalación autorizada cuando falta usa el último release estable oficial de GitHub y su metadata viva, no un pin estático. Consultar `docs/references/pi-runtime.md` para transiciones y rollback, sin editar receipts ni hashes, ni borrar HOME, Engram o proyecciones ajenas.

...
Stack gestiona la instalación de JorgeX Pi; Pi posee su proyección Pi-native, bootstrap, contratos, dependencias y lifecycle. `src/lib/pi-runtime-pin.json` es la autoridad de la identidad del paquete, procedencia y digests del tarball; `src/lib/pi-runtime.ts` conserva el lifecycle y sus contratos. Nunca usar `latest`. Consultar `docs/references/pi-runtime.md` para transiciones y rollback: usar versiones que reconozcan cada receipt, sin editar receipts ni hashes, ni borrar HOME, Engram o proyecciones ajenas.
...
La publicación de Stack usa el auto-bump existente al mergear y selecciona el patch disponible; no se debe afirmar ni preparar manualmente un número de release futuro. Una vez publicado y verificado el artefacto Pi, su consumo puede comenzar de inmediato; el pin exacto, la procedencia, los digests y la compatibilidad siguen siendo obligatorios. El lector `.github/scripts/pi-pin.mjs` valida el JSON y expone URL y tamaño al workflow; los digests y la procedencia se consumen desde el JSON. La attestation externa de npm queda fuera del runtime y `provenance.commit` es informativo salvo verificación explícita.
...
Con la App configurada y `JORGEX_AUTOMATION_ENABLED=true`, el coordinador de Stack reconcilia ambos sentidos desde `main` o dispatch: propone la snapshot Pi del canon y la adopción en Stack del paquete Pi publicado y verificado, con no-op si no hay cambios. Antes de preparar una PR manual, comprobar runs y PRs existentes para evitar duplicados; consultar el [runbook Stack ↔ Pi](docs/references/stack-pi-automation.md). Sin opt-in, ante incompatibilidad o fallo explícito, usar el preparador local y una PR manual con review y gates. Un artefacto pendiente sigue siendo una dependencia externa: no editar pines ni hashes a mano por rutina ni forzar fixtures para aceptar cambios semánticos; estos requieren revisión. La adopción prepara `src/lib/pi-runtime-pin.json` y `tests/fixtures/pi-runtime-artifacts.json`; el lifecycle y los contratos siguen en `src/lib/pi-runtime.ts`. El merge requiere siempre orden de Jorge; la automatización no hace auto-merge ni instalaciones personales.
...
`upstreams.json` conserva source/path y commit aceptado de cada skill vendorizada. Solo un mantenedor desde un clon git puede proponer su actualización: descarga temporal, diff visible y confirmación antes de reemplazar/re-pin; `modified: true` requiere confirmación adicional. Para skills, `update --check` en un paquete instalado es discovery-only: no consulta ni ejecuta upstreams. La sección `complements` unifica los pins exactos de complementos oficiales externos que ejecutan código (plugins Engram, `gentle-engram`, `pi-mcp-adapter`, DevTools MCP, Playwright CLI): al instalar se usa siempre el pin, nunca `latest` ni rangos flotantes; `update --check` los revisa en toda máquina —también usuario final— (discovery-only, sin auto-update) y avisa si difieren del pin, y la adopción es re-pin deliberado con verificación. Revisar `reviewed` cuando se toque cualquiera de esos complementos. La limpieza de componentes retirados solo elimina rutas propias según manifest, con backup incluso si se modificaron; conserva rutas ajenas. Nunca tocar vaults ni binarios de Obsidian al retirar sus skills.

...
El pin de este checkout es el publicado y verificado `npm:jorgex-pi@0.8.7`; `package.json` identifica la versión base de Stack `1.9.19`. La publicación de Stack usa el auto-bump existente al mergear y selecciona el patch disponible; no se debe afirmar ni preparar manualmente un número de release futuro. La versión publicada `1.9.19` reconoce el receipt de Pi `0.8.6`; la nueva versión de Stack que consuma este pin debe comprobarse en el registro después de publicarse y confirmar que reconoce el receipt de `0.8.7`. La barrera gestionada de 24 h en npm afecta al consumo/instalación real del paquete Pi nuevo, no a validación, merges ni releases. Tamaño y hashes se verifican según `src/lib/pi-runtime.ts` y `tests/fixtures/pi-runtime.ts`; la attestation externa de npm queda fuera del runtime y `provenance.commit` es informativo salvo verificación explícita.

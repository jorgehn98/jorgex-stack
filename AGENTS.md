...
El checkout documentado prepara Stack `1.9.18` con el pin publicado y verificado `npm:jorgex-pi@0.8.6`. Stack `1.9.17` reconoce el receipt de Pi `0.8.5` y Stack `1.9.18` el de `0.8.6`; antes de consumir cualquiera de las dos versiones, comprueba su existencia en el registro. La barrera gestionada de 24 h en npm afecta al consumo/instalación real del paquete Pi nuevo, no a validación, merges ni releases. Tamaño y hashes se verifican según `src/lib/pi-runtime.ts` y `tests/fixtures/pi-runtime.ts`; la attestation externa de npm queda fuera del runtime y `provenance.commit` es informativo salvo verificación explícita.

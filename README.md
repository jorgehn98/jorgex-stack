...
The published Stack `1.9.7` recognizes the exact Pi receipt `npm:jorgex-pi@0.8.4`. The Pi `0.8.5` adoption is future Stack `1.9.8` work and requires merge, publication and managed maturity; use exact versions, never `latest`, and never edit receipts or hashes or delete `HOME`, Engram, or another runtime's projection to force trust. The transition and rollback commands are in [docs/references/pi-runtime.md](docs/references/pi-runtime.md).

Stack 1.9.7 → Pi 0.8.4 is the current published pairing. Stack 1.9.8 → Pi 0.8.5 is a future transition, only after Stack publication and the Pi 0.8.5 managed-maturity window. The 24-hour managed-consumption maturity rule applies only to real installation or consumption of the new Pi package; development, PR validation, merge and Stack publication may proceed immediately. Installing it on a real user scope before the maturity window requires Jorge's explicit exception.

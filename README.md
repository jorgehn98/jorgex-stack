...
- **Playwright CLI** (recommended): the global `@playwright/cli` package, shared by the machine and enabled explicitly. Stack resolves and verifies the exact published release, then installs it and verifies Chromium before recording the observed version. The conditional browser guide tells agents to open Chromium with `--browser=chromium`, consult `playwright-cli --help`, use a task-specific session, take a `snapshot`, verify results and close only sessions they created. See [docs/references/browser-automation.md](docs/references/browser-automation.md) for the lifecycle, privacy profile and troubleshooting.
- **Chrome DevTools MCP** (advanced diagnostics, opt-in): disabled by default and selected per runtime. Stack resolves the published release, verifies its SRI and tests the four mandatory flags in an isolated stage before writing the observed version into the runtime command `pnpm dlx chrome-devtools-mcp@<observed-version> --isolated --redact-network-headers --no-performance-crux --no-usage-statistics`. The full-mode tool count and schema cost depend on the release. `--isolated` starts Chrome with an ephemeral profile deleted on close; `--redact-network-headers` redacts sensitive headers, not request/response bodies. Avoid authenticated sessions or sensitive data. `--no-performance-crux` disables CrUX reporting; `--no-usage-statistics` disables telemetry. `--slim` and Playwright MCP are excluded.
...
Under the hood, `--playwright` verifies the npm metadata and tarball SRI, then runs two `pnpm` argv-only plans for that exact version: `pnpm add --global @playwright/cli@<observed-version>` and `pnpm dlx @playwright/cli@<observed-version> install-browser chromium`. It checks the installed package version and verifies that Chromium launches headless against `about:blank` before saving the observed version and integrity. Removal is `pnpm remove --global @playwright/cli` (no version suffix). If installation fails, the error identifies the failed phase and recommends `jorgex-stack install --playwright`; the preference is not marked enabled unless the complete plan succeeds.
...
- `update --check` only inspects Playwright CLI when its preference is `enabled` (a binary appearing in `PATH` is not consent). It compares the installed version against the local observed version without querying npm or installing anything; a Playwright CLI binary-only update does not require `sync` afterwards.
...
- `install`/`sync` also inject (and `disable`/`uninstall` remove) managed capability sections in `AGENTS.md` (OpenCode, Codex) or `CLAUDE.md` (Claude Code). File runtimes keep `jorgex:playwright`, `jorgex:chrome-devtools` and `jorgex:context7` independent; compatible Pi packages project Context7 after registering an isolated HTTP bridge. Stack does not add an Engram protocol section or filter provider tools. User content outside managed markers is preserved. `--target-dir` never reads real preferences; explicit `--devtools` only simulates the MCP entry inside the temporary target when the harness injects a validated observation. `install --dry-run --playwright` previews the Playwright guide without installing anything. Ambiguous system-prompt markers block writes. See [docs/references/browser-automation.md](docs/references/browser-automation.md) §2.7.
...
3. **Playwright CLI** (only when explicitly enabled): compares the detected binary with the recorded observation, reports the published version in the interactive picker and requires a second confirmation. Then it resolves and verifies the exact package, updates the global install and downloads Chromium using the same candidate; failure in either phase blocks completion. The error identifies the failed phase and recommends `jorgex-stack install --playwright` to retry; a Playwright update does not require `sync`.

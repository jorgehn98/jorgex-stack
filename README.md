...
Pi combines the frozen **snapshot v2** package with a Stack-owned shared projection. The current published Stack `1.9.3` still recognizes the outgoing exact package **`jorgex-pi@0.8.0`**. This checkout carries the next production candidate **`jorgex-pi@0.8.3`**; its Stack release is not published yet, so no future Stack version is named here.

The following command set is retained from the historical published Stack `1.9.2`; it is not a command set for the unpublished candidate release.
...
The outgoing published artifact records two distinct commit identities: the release checkout and tarball producer is `9f999747df3e335947a61d38e581555367973b09` (`main`, release `0.8.0`); and the Stack parity source is `11e7666ea4e40bde1de8bc434610747eb797ab9c`. For the new candidate, the verified Pi release checkout is `0a35c283fe30a9fed87da3cedc00bab97163e68b` and the parity source is `fac3ba828c20134858e2a9aa1e68b920270f7b5a`. Registry metadata has no `gitHead`; README does not invent a separate source identity, attestation or signature.
...
The published Stack `1.9.3` recognizes the outgoing exact Pi receipt for `npm:jorgex-pi@0.8.0`. This checkout's production candidate is `npm:jorgex-pi@0.8.3`, but the corresponding new Stack release is not published yet; use exact versions, never `latest`, and never edit receipts or hashes or delete `HOME`, Engram, or another runtime's projection to force trust. The historical rollback examples are in [docs/references/pi-runtime.md](docs/references/pi-runtime.md).

Stack 1.9.3 is the verified published Stack release. The new Pi 0.8.3 artifact has independently verified public registry bytes, but its adopting Stack release still requires post-merge publication and public npm readback before a new Stack version or transition command is documented. The 24-hour managed-consumption maturity rule applies only to real installation or consumption of the new Pi package; development, PR validation, merge and Stack publication may proceed immediately. Installing it on a real user scope before 24 hours requires Jorge's explicit exception.
